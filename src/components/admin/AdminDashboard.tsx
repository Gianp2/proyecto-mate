...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Helper to determine low inventory
  ...
  const handleOpenProductModal = (product?: Product) => {
    if (product) {
      ...
      setEditingProduct({
        ...
        featured: false,
        active: true,
        features: ['Calidad artesanal garantizada', 'Material premium'],
        variants: [],
      });
    }
    setIsModalOpen(true);
  ...
  // Save Settings
  const handleSaveSettings = async (form: Partial<StoreSettings>) => {
  ...
  const handleExportCatalog = () => {
    ...
    downloadAnchor.setAttribute('download', `catalogo_pampa_${new Date().toISOString().slice(0, 10)}.json`);
  ...
  const totalInventoryValue = products.reduce((acc, p) => acc + p.price * (p.stockQuantity ?? 1), 0);
      ...
      <main className="flex-1 p-3.5 sm:p-6 lg:p-8 pb-28 md:pb-8 overflow-y-auto overflow-x-hidden min-w-0">
        ...
        {activeTab === 'products' && (
          <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5">
              <div>
                ...
                <p className="text-xs sm:text-sm text-[#7C6E65] dark:text-[#A39489] mt-0.5">
                  Gestioná precios, stock, fotos con Cloudinary y visibilidad de tus mates y accesorios.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3.5 py-3 rounded-2xl bg-white dark:bg-[#241E1B] hover:bg-[#FAF8F5] dark:hover:bg-[#2E2622] text-[#5C4F48] dark:text-[#C5BBB4] border border-[#EBE6DD] dark:border-[#3D322B] text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-2xs transition-colors"
                ...
                <button
                  ...
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#4B5A36] hover:bg-[#3A4729] dark:bg-[#809761] dark:hover:bg-[#6b824e] text-white dark:text-[#181412] font-semibold text-xs sm:text-sm px-5 py-3 rounded-2xl shadow-xs transition-all active:scale-95 cursor-pointer shrink-0 min-h-[44px]"
                >
            ...
            {/* Quick Metrics Bar on top of products */}
            ...
            {lowStockProducts.length > 0 && (
              <div className="bg-amber-50/90 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800/60 rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs">
                <div className="flex items-start sm:items-center gap-3.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-800 dark:text-amber-300 mt-0.5">
                      Tenés {lowStockProducts.length} {lowStockProducts.length === 1 ? 'producto' : 'productos'} con inventario bajo (&lt; 3 unidades o agotados).
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <span>{onlyLowStockFilter ? 'Mostrar Todos' : 'Filtrar Críticos'}</span>
                </button>
              </div>
            )}
      ...
      >
        <div className="grid grid-cols-4 gap-2 max-w-md mx-auto items-center">
          ...
          >
            ...
            <span className="text-[11px] mt-1 tracking-tight">Crear</span>
          </button>
